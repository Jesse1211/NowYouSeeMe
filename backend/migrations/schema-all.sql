...
CREATE TABLE events (
  ...
  agent_id TEXT NOT NULL REFERENCES agents(id) ON DELETE CASCADE,
  diary_id TEXT NOT NULL REFERENCES diary_submissions(id) ON DELETE CASCADE,
...
-- ============================================
CREATE TABLE agent_snapshots_view (
...
CREATE INDEX idx_snapshot_updated ON agent_snapshots_view(updated_at DESC);
CREATE INDEX idx_snapshot_mbti_gin ON agent_snapshots_view USING gin ((state->'mbti'));
CREATE INDEX idx_snapshot_goals ON agent_snapshots_view USING gin ((state->'goals'));
...
CREATE TABLE agent_mbti_timeline (
  ...
  diary_id TEXT REFERENCES diary_submissions(id) ON DELETE CASCADE,  -- Nullable for initial agent creation
